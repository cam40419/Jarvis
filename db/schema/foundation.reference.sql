...
CREATE TYPE risk_class AS ENUM ('read', 'write_soft', 'write_hard', 'dangerous');

CREATE TYPE job_status AS ENUM ('queued', 'running', 'waiting', 'needs_human', 'succeeded', 'failed', 'cancelled');
...
CREATE TABLE threads (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    household_id uuid NOT NULL REFERENCES households(id),
...
CREATE TABLE messages (
    ...
    channel text NOT NULL CHECK (channel IN ('api', 'chat', 'voice', 'scheduler', 'worker')),
    created_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX messages_thread_time_idx ON messages (
    thread_id,
    created_at, id
);
...
CREATE INDEX memories_scope_idx ON memories (household_id, scope, kind);
...
CREATE TABLE jobs (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (household_id, kind, idempotency_key)
);
...
CREATE TABLE confirmations (
    ...
    risk risk_class NOT NULL CHECK (risk IN ('write_hard', 'dangerous')),
...
CREATE TABLE audit_events (
    ...
    UNIQUE (household_id, event_hash)
);
