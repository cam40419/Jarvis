...
class PostgresStore(InMemoryStore):
    ...
    def home_commands(
        ...
        with self.transaction():
            ...
            params: list[UUID] = [household_id, actor_id]
            if run_id is not None:
                query += " AND run_id = %s"
                params.append(run_id)
            rows = self.connection.execute(
                query + " ORDER BY created_at DESC, id DESC LIMIT 100", params
            ).fetchall()
    ...
    def google_connection(self, household_id: UUID, actor_id: UUID) -> GoogleConnection | None:
        with self.transaction():
            ...
            return GoogleConnection.model_validate(row["snapshot"]) if row else None

    def save_google_connection(self, connection: GoogleConnection) -> None:
        with self.transaction():
            self.connection.execute(
                "INSERT INTO google_connections (household_id, actor_id, snapshot) "
                "VALUES (%s, %s, %s) "
                "ON CONFLICT (household_id, actor_id) DO UPDATE SET snapshot = EXCLUDED.snapshot",
                (
    ...
    def save_action(self, action: ActionProposal) -> None:
        with self.transaction():
            self.connection.execute(
                "INSERT INTO action_proposals (id, household_id, actor_id, run_id, snapshot) "
                "VALUES (%s, %s, %s, %s, %s) "
                "ON CONFLICT (id) DO UPDATE SET snapshot = EXCLUDED.snapshot",
                (
                    action.id,
